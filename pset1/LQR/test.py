...
def test_compute_Q_params():
    ...
    for seed, data in all_data.items():
        ...
        np.random.seed(seed)
        n_x, n_u = data['n_x'], data['n_u']

        A = np.random.rand(n_x, n_x)
        B = np.random.rand(n_x, n_u)
        m = np.random.rand(n_x, 1)
        Q = rand_pd_matrix(n_x)
        R = rand_pd_matrix(n_u)
        M = np.random.rand(n_x, n_u)
        q = np.random.rand(n_x, 1)
        r = np.random.rand(n_u, 1)
        b = np.random.rand(1)
        P = rand_pd_matrix(n_x)
        y = np.random.rand(n_x, 1)
        p = np.random.rand(1)
...
def test_compute_V_params():
    ...
    for seed, data in all_data.items():
        ...
        B = np.random.rand(n_x, n_u)
        m = np.random.rand(n_x, 1)
        C = rand_pd_matrix(n_x)
        D = rand_pd_matrix(n_u)
        E = np.random.rand(n_x, n_u)
        f = np.random.rand(n_x, 1)
        g = np.random.rand(n_u, 1)
        h = np.random.rand(1)
        K = np.random.rand(n_u, n_x)
